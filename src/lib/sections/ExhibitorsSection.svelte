<script>
  ...
  let exhibitors = $state(loadExhibitors());
  ...
  function handleSubmit(e) {
    e.preventDefault();
    if (!formName.trim()) {
      formError = 'Exhibitor name is required.';
      return;
    }
    const newExhibitor = {
...
<div class="exhibitors__module">
  <div class="exhibitors__toolbar">
    ...
    <button class="exhibitors__add-btn" type="button" onclick={openModal}>Add Exhibitor</button>
  </div>
...
<Modal open={isModalOpen} title="Add Exhibitor" onClose={closeModal}>
  <form class="exhibitor-form" onsubmit={handleSubmit}>
    <label class="exhibitor-form__field">
      <span class="exhibitor-form__label">Name</span>
      <input class="exhibitor-form__input" type="text" placeholder="Company name" bind:value={formName} />
    </label>
    <label class="exhibitor-form__field">
      <span class="exhibitor-form__label">Category</span>
      <input class="exhibitor-form__input" type="text" placeholder="e.g. Software" bind:value={formCategory} />
    </label>
    <label class="exhibitor-form__field">
      <span class="exhibitor-form__label">Booth</span>
      <input class="exhibitor-form__input" type="text" placeholder="e.g. A12" bind:value={formBooth} />
    </label>
    <label class="exhibitor-form__field">
      <span class="exhibitor-form__label">Status</span>
      <select class="exhibitor-form__input" bind:value={formStatus}>
    ...
    {#if formError}
      <p class="exhibitor-form__error">{formError}</p>
    {/if}

    <div class="exhibitor-form__actions">
      <button type="button" class="exhibitor-form__cancel" onclick={closeModal}>Cancel</button>
      <button type="submit" class="exhibitor-form__submit">Add Exhibitor</button>
    </div>
  </form>
</Modal>
